...
export function useShoppingCart<T extends Record<string, unknown>>(
    ...
    const updateItem = React.useCallback(
        (key: string | number, patch: Partial<T>) => {
            setItems(prev =>
                prev.map(item => getItemKey(item) === key ? { ...item, ...patch } : item)
            )
        }
    , [getItemKey]);
    ...
    const getSubtotal = React.useCallback(
        () => items.reduce((acc, item) => acc + getItemPrice(item) * getItemQuantity(item), 0),
        [items, getItemPrice, getItemQuantity]
    );
    
    const getTotalTax = React.useCallback(
        () => getItemTax ? 
            items.reduce((acc, item) => acc + getItemTax(item) * getItemQuantity(item), 0) :
            0,
    ...
    const getTotalDiscount = React.useCallback(
        () => getItemDiscount ?
            items.reduce((acc, item) => acc + getItemDiscount(item) * getItemQuantity(item), 0) :
            0,
    ...
    }, [items, getItemKey, getItemPrice, getItemQuantity, getItemTax, getItemDiscount]);
